...
angular.module('testApp')
  .controller('NewsMnaCtrl', function ($rootScope, $scope, $state, $filter, $location, $window, $timeout, $log, $http,
                                          $localStorage, NewsFactory, News, Company, Person, Product, Mna, Event, Category, timeFactory) {


    // or 조건으로 검색 되는 fields 리스트
    $scope.generateWhereOrList = ['takeoverId'];
    // or 조건으로 검색 되는 fields 리스트
    ...
    if ($state.current.name == 'mna') {
      $scope.infiniteBreak2 = false;
    } else {
      $scope.infiniteBreak2 = true;
    }
