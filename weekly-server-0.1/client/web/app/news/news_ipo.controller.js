...
angular.module('testApp')
  .controller('NewsIpoCtrl', function ($rootScope, $scope, $state, $filter, $location, $window, $timeout, $log, $http,
                                       $localStorage, NewsFactory, News, Company, Person, Product, Ipo, Event, Category, timeFactory) {


    // or 조건으로 검색 되는 fields 리스트
    $scope.generateWhereOrList = ['companyId'];
    // or 조건으로 검색 되는 fields 리스트
    ...
    if ($state.current.name == 'ipo') {
      $scope.infiniteBreak3 = false;
    } else {
      $scope.infiniteBreak3 = true;
    }
