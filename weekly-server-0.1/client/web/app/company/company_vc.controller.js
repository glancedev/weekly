...
angular.module('testApp')
  .controller('CompanyVcCtrl', function ($rootScope, $scope, $state, $location, $window, $timeout, $log, $http,
                                              $localStorage, NewsFactory, News, Company, Person, Category, Investment, timeFactory) {

    // or 조건으로 검색 되는 fields 리스트
    $scope.generateWhereOrList = [];
    ...
    $scope.loadMore = function(){
      ...
      $scope.generateWhere()
        .then(function(filter){
          ...
          if(null == filter.filter.where.and) filter.filter.where.and = [{type : "투자기관"}];
          else filter.filter.where.and.push({type : "투자기관"});                     
          ...
          Company
            ...
            .then(
              function (res) {
                ...
                angular.forEach($scope.contents, function(value, i){
                  $scope.getInvestment(i, value.id);
                });                
     ...
     *  News 리스트 가져오기
     **/
    $scope.getInvestment = function(index, id){
      // 검색 조건 생성
        Investment
        .find({
            filter:{        
                fields: ['id', 'investeeId'],
                include:[
                    {
        ...
        .then(
        function (res) {
            $scope.contents[index].InvestmentCnt = res.length;
            $scope.contents[index].Investment = $scope.removeDuplicates(res, 'investeeId');
        },
        function (err) {                
            console.log('err', err);
            alert(err.data.error.message);
