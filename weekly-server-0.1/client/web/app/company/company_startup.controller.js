...
angular.module('testApp')
  .controller('CompanyStartupCtrl', function ($rootScope, $scope, $state, $location, $window, $timeout, $log, $http,
    ...
    $scope.generateWhereCompareList = ['foundationDate_gte', 'foundationDate_lte', 'totalMoney_gte', 'totalMoney_lte'];
    ...
    $scope.reset = function(){
      ...
      $scope.where = {
        state : [],   // 상태
        field : [],  // 분야 태그
        totalMoney_gte : [], //누적 투자액
        totalMoney_lte : [], //누적 투자액
        founderSchool : [], // 대표 출신 학교
    ...
    $scope.loadMore = function(){
      ...
      $scope.generateWhere()
        .then(function(filter){
          ...
          if(null == filter.filter.where.and) filter.filter.where.and = [{type : "스타트업"}];
          else filter.filter.where.and.push({type : "스타트업"});                     
    ...
    $rootScope.putTags = $scope.putTags = function(group, item, alias, load){
      ...
      else
      {
        ...
        var name = item;
        if(null != alias) name = alias;

        // group list에 item 검색 조건 추가
        $scope.searchTools.list.push({group:group, name:name}); //검색바화면 표시용
    ...
    **/
    ...
    $scope.sub_menu3 = [
      { title: '신규 설립 회사', icon: '', link: '#', group: 'foundationDate_gte', id: null, name: '3개월 이내_'+$scope.before3month, alias: null },
      { title: '100억원 이상 투자 유치', icon: '', link: '#', group: 'totalMoney_gte', id: null, name: '100억원 이상_'+100, alias: null },
      { title: '핀테크 분야 회사', icon: '', link: '#', group: 'field', id: null, name: '핀테크', alias: null },
      { title: 'VR/AR 분야 회사', icon: '', link: '#', group: 'field', id: null, name: 'VR/AR', alias: null },
      { title: '미국 소재 회사', icon: '', link: '#', group: 'location,location', id: null, name: '미국(서부),미국(동부)', alias: null },
      { title: '중국 소재 회사', icon: '', link: '#', group: 'location', id: null, name: '중국', alias: null },
      { title: 'KAIST출신 창업자 회사', icon: '', link: '#', group: 'founderSchool', id: null, name: '카이스트', alias: null }
    ];
